...
describe("custom attributes (APP-14941)", () => {
  ...
  it("emits the customer's display fields beside the identifier, at the level the identifier rides", () => {
    ...
    // The default customer is a DIFFERENT customer from the root's, so its
    // display fields stay off the resource: ingest reads root over resource
    // per attribute, and `default.co` would otherwise become cus_1's domain.
    const resource = resourceAttrs(body);
    expect(resource["glassray.customer"]).toBe("default-co");
    expect(resource["glassray.customer.domain"]).toBeUndefined();
    expect(resource["glassray.customer.name"]).toBeUndefined();
  });

  it("emits the default customer's display fields when the trace is for that customer, or names none", () => {
    const defaults = cfg({
      customer: "default-co",
      customerProfile: { name: "Default Co", email: undefined, domain: "default.co" },
    });
    const same = serializeTrace(
      trace([span({ isRoot: true, kind: "agent" })], { customer: "default-co" }),
      defaults,
      noWarn,
    );
    expect(resourceAttrs(same)["glassray.customer.domain"]).toBe("default.co");
    const none = serializeTrace(trace([span({ isRoot: true, kind: "agent" })]), defaults, noWarn);
    expect(resourceAttrs(none)["glassray.customer.domain"]).toBe("default.co");
  });
